...
const { getBuffer } = require('./lib/funct')
const sock = new WAConnection
const time = moment.tz('Asia/Jakarta').format('DD/MM HH:mm:ss')

require('./socket.js')
nocache('./socket.js', module => console.log(`${module} is now updated!`))
...
async function starts () {
    ...
    sock.on('group-participants-update', async (anu) => {
        ...
        if (anu.action == 'add') {
            ...
            sock.sendMessage(anu.jid, textwel, MessageType.text, { contextInfo: { externalAdReply: { title: "Welcome Message", body: `${member.length} Members`, previewType: "PHOTO", jpegThumbnail: getBuffer(ppuser), sourceUrl: `` }, mentionedJid: [mem] }})
        } else if (anu.action == 'remove') {
        	textlev = `Selamat tinggal @${mem.split("@")[0]}`
            sock.sendMessage(anu.jid, textlev, MessageType.text, { contextInfo: { externalAdReply: { title: "Leave Message", body: `${member.length} Members`, previewType: "PHOTO", jpegThumbnail: getBuffer(ppuser), sourceUrl: `` }, mentionedJid: [mem] }})
        } else if (anu.action == 'promote') {
        	msg = sock.prepareMessageFromContent(mdata.id, { "orderMessage": { "itemCount": 404, "message": `- [ PROMOTE DETECTED ] -\n\n• Nomor : @${mem.split("@")[0]}\n• Time : ${time}\n Type : Promote\n\nSelamat anda sekarang menjadi admin`, "footerText": `Promote Message`, "thumbnail": fs.readFileSync('./media/rzxbotneon.jpg'), "surface": 'CATALOG' }}, { contextInfo: { mentionedJid: [mem] }})
			sock.relayWAMessage(msg)
        } else if (anu.action == 'demote') {
        	msg = sock.prepareMessageFromContent(mdata.id, { "orderMessage": { "itemCount": 404, "message": `- [ DEMOTE DETECTED ] -\n\n• Nomor : @${mem.split("@")[0]}\n• Time : ${time}\n Type : Demote\n\nYah sekarang anda menjadi member`, "footerText": `Demote Message`, "thumbnail": fs.readFileSync('./media/rzxbotneon.jpg'), "surface": 'CATALOG' }}, { contextInfo: { mentionedJid: [mem] }})
			sock.relayWAMessage(msg)
        }
    })
