...
async function starts () {
    ...
    sock.on('chat-update', async (msg) => {
		...
		await funct.serialize(sock, msg)
    ...
    sock.on('group-participants-update', async (anu) => {
        ...
        if (anu.action == 'add') {
            ...
            sock.sendMessage(anu.jid, textpro, MessageType.text, { contextInfo: { externalAdReply: { title: "Promote Message", body: `Rzxbot`, previewType: "PHOTO", jpegThumbnail: getBuffer(ppuser), sourceUrl: `` }, mentionedJid: [mem] }})
        } else if (anu.action == 'demote') {
        	textdem = `@${mem.split("@")[0]} anda bukan admin lagi!`
            sock.sendMessage(anu.jid, textdem, MessageType.text, { contextInfo: { externalAdReply: { title: "Demote Message", body: `Rzxbot`, previewType: "PHOTO", jpegThumbnail: getBuffer(ppuser), sourceUrl: `` }, mentionedJid: [mem] }})
        }
    })
