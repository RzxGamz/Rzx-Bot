    ...
    setInterval(() => {
        ...
        if (position !== null) {
            console.log(`Sewa expired: ${_dir[position].id}`)
                xinz.sendMessage(_dir[position].id, `Waktu sewa di grup ini sudah habis, bot akan keluar otomatis`, MessageType.text)
                .then(() => {
                    xinz.groupLeave(_dir[position].id)
